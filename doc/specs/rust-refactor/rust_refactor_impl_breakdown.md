...
Rewrite the **core** in Rust (scheduler, edges, event/segment model, group
supervisor, control protocol, the 4 Teams, and the seek/timeline logic currently
buried in `input_rec`). Reimplement **~4 "nodes" that are really core concerns**
(`realtime`, `speed`, `pause`, plus `input_rec`'s seek/timeline half) in Rust as
core services. Write **~8 native Rust nodes** — the trivial + pure-plumbing set — as
the proof/dogfood set. Leave the **other ~80 node types in C++**, recompiled
against a new `node_common.hpp` shim, **unchanged at the source level in the common
case**, forever. CUDA/TensorRT/GL/JACK/OBS/Python nodes are never ported.

So: **Rust = core + ~4 services + ~8 nodes. C++ = ~80 nodes, permanently.**
...
|---|---|---|---|
| Scheduler, edges (`Buffer\|Event`, sticky replay), executors, group supervisor, control protocol, factory registry, `query_interface`, `SyncGroup`/epoch | **Rust (new)** | The core | the bulk of new code |
...
|---|---|---|
| `realtime.cpp` | 568 | executor clock domain + `SyncGroup`; emits `Segment` |
| `speed.cpp` | 278 | `SpeedControlTeam` service; `scalePTS` → `Segment{rate}` |
| `pause.cpp` | 97 | `PauseControlTeam` service; executor group-suspend |
| `input_rec.cpp` (seek/timeline half) | ~600 of 1202 | `SeekIndex` + `Segment` emitter (§6.3 of design) |
...
| `picture_buffer_sink.cpp` | 43 | simple sink |
...
- `DECLNODE*` macros — expand to `avp_register_node_factory(...)` + an auto-generated
  `query_interface` that maps the closed interface-ID table to `dynamic_cast<IFoo*>`.
- The capability interfaces (`IDecoder`, `IEncoder`, `IVideoFormatSource`, …) —
...
Nothing changes. It keeps `av::VideoRescaler`, `av_dict_copy`, the `nb_side_data`
walk, and `findNodeUp<IPreferredFormatReceiver>()`. The shim delivers an `AVFrame*`
wrapped as `av::VideoFrame`; `findNodeUp` routes through `avp_find_interface_up`.
This is the proof that avcpp-heavy nodes need no rewrite.
...
- **`IFlushAndSeek` overriders** — the old 4-phase base methods are gone from the
  shim base; nodes now receive `FlushStart`/`Segment` events instead. Grep shows the
...
|---|---|---|---|
| **M0** | `avplumber_core.h` + `EdgeItem`/`Segment` structs; 2-node smoke graph (demux→null_sink) over the ABI | Rust + C header | 0 |
| **M1** | Core MVP: graph, `BufferedEdge` (+sticky replay), blocking-thread sched, one executor, control protocol, factory registry, group supervisor, `query_interface`, `find_interface_up` | Rust | — |
...
| **M3** | Event/segment playback: `FlushStart/Stop/Segment/Caps`, `SyncGroup`, epoch; rewrite the 4 Teams + `input_rec` seek/timeline in Rust; cut pipeline over; delete `IFlushAndSeek` | Rust | Tier C live |
| **M4** | Native node API: `Transform`/`FlowNode` traits, `#[avp_node]` proc macro, `linkme` registry; port Tier-R proof set | Rust | +8 Rust nodes |
...
- **New Rust core (M1)** — the real work: scheduler, edges+sticky replay, executors,
  supervisor, protocol, registry, interface bridge.
- **Rust services (M3)** — ~2.1k C++ LOC of Team + realtime/speed/input_rec logic
  *re-expressed* (not line-ported) onto events/segments. Conceptually the hardest,
  because it's the seek/sync correctness surface.
- **C++ shim (M2)** — one header + marshalling; bounded, high-leverage (unlocks ~80
  nodes at once).
- **Rust nodes (M4)** — ~8 small nodes, mostly to validate the API and macro.
- **Never touched** — ~55 CUDA/TensorRT/GL/JACK/OBS/Python nodes; they only need to
  recompile against the shim.

**Bottom line on "how many Rust nodes":** ~4 dissolve into the core, ~8 become native
Rust, ~80 stay C++ behind the shim (of which ~55 permanently). The Rust node count is
deliberately small — the win is the core + event model + boilerplate reduction, not
rewriting working media nodes.
