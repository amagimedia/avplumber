...
impl Node for StubSource {
    ...
    fn process(&self) -> Result<Processed, NodeError> {
        ...
        match sink.push(buf) {
            ...
            Push::Full => Ok(Processed::Again),
            Push::Closed | Push::Dropped => Ok(Processed::Done),
        }
    }
}
...
impl Node for StubSink {
    ...
    fn process(&self) -> Result<Processed, NodeError> {
        ...
        match source.take(-1) {
            ...
            Some(EdgeItem::Event(EdgeEvent::Eof)) => Ok(Processed::Done),
            Some(_) => Ok(Processed::Again),
...
fn direct_edge_rejects_blocking_or_fallible_endpoints() {
    ...
    match result {
        Err(avplumber_f7k::CoreError::Invalid(msg)) => {
            assert!(msg.contains("direct_consumer_is_infallible"), "{msg}");
        }
        Err(other) => panic!("expected Invalid, got {other}"),
