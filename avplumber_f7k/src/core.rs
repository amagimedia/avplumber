...
impl Instance {
    ...
    pub fn connect_edge(
        ...
        if matches!(coupling, EdgeKind::Direct) && !consumer_node.direct_consumer_is_infallible() {
            return Err(CoreError::Invalid(format!(
                "DirectEdge consumer `{consumer}` must explicitly opt in as an infallible Direct consumer with `Node::direct_consumer_is_infallible`; use BufferedEdge for a fallible consumer"
            )));
        }
