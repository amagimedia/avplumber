#!/usr/bin/env bash
# Generate deps/ffmpeg/8/0011-avformat-libmxl-demuxer-muxer.patch by
...
log "applying the existing series (everything below 0011)"
# A previous 0011 in $PATCH_DIR is what we are about to replace, so skip it.
mapfile -t EXISTING < <(ls "$PATCH_DIR"/0[0-9][0-9][0-9]-*.patch | sort | grep -v '/0011-')
