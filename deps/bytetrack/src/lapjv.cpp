	...
	int_t *free_rows, int_t *x, int_t *y, cost_t *v)
{
	...
	for (uint_t i = 0; i < n; i++) {
		for (uint_t j = 0; j < n; j++) {
			...
			LAPJV_PRINTF("i=%d, j=%d, c[i,j]=%f, v[j]=%f y[j]=%d\n", i, j, c, v[j], y[j]);
		}
	}
	LAPJV_PRINT_COST_ARRAY(v, n);
	LAPJV_PRINT_INDEX_ARRAY(y, n);
	LAPJV_NEW(unique, boolean, n);
	memset(unique, LAPJV_TRUE, n);
	{
		int_t j = n;
		do {
			j--;
	...
	int_t *free_rows, int_t *x, int_t *y, cost_t *v)
{
	...
	uint_t rr_cnt = 0;
	LAPJV_PRINT_INDEX_ARRAY(x, n);
	LAPJV_PRINT_INDEX_ARRAY(y, n);
	LAPJV_PRINT_COST_ARRAY(v, n);
	LAPJV_PRINT_INDEX_ARRAY(free_rows, n_free_rows);
	while (current < n_free_rows) {
		...
		rr_cnt++;
		LAPJV_PRINTF("current = %d rr_cnt = %d\n", current, rr_cnt);
		...
		if (rr_cnt < current * n) {
			...
			LAPJV_PRINTF("rr_cnt=%d >= %d (current=%d * n=%d)\n", rr_cnt, current * n, current, n);
			if (i0 >= 0) {
				free_rows[new_free_rows++] = i0;
			}
		}
	...
	int_t *pred)
{
	...
	LAPJV_NEW(cols, int_t, n);
	LAPJV_NEW(d, cost_t, n);
	...
	LAPJV_PRINT_COST_ARRAY(d, n);
	while (final_j == -1) {
		// No columns left on the SCAN list.
		if (lo == hi) {
			LAPJV_PRINTF("%d..%d -> find\n", lo, hi);
			n_ready = lo;
			hi = _find_dense(n, lo, d, cols, y);
			LAPJV_PRINTF("check %d..%d\n", lo, hi);
			LAPJV_PRINT_INDEX_ARRAY(cols, n);
		...
		if (final_j == -1) {
			LAPJV_PRINTF("%d..%d -> scan\n", lo, hi);
			final_j = _scan_dense(
				n, cost, &lo, &hi, d, cols, pred, y, v);
			LAPJV_PRINT_COST_ARRAY(d, n);
			LAPJV_PRINT_INDEX_ARRAY(cols, n);
			LAPJV_PRINT_INDEX_ARRAY(pred, n);
		}
	}

	LAPJV_PRINTF("found final_j=%d\n", final_j);
	LAPJV_PRINT_INDEX_ARRAY(cols, n);
	{
	...
	int_t *free_rows, int_t *x, int_t *y, cost_t *v)
{
	...
	LAPJV_NEW(pred, int_t, n);

	for (int_t *pfree_i = free_rows; pfree_i < free_rows + n_free_rows; pfree_i++) {
		int_t i = -1, j;
		uint_t k = 0;

		LAPJV_PRINTF("looking at free_i=%d\n", *pfree_i);
		j = find_path_dense(n, cost, *pfree_i, y, v, pred);
		LAPJV_ASSERT(j >= 0);
		LAPJV_ASSERT(j < (int_t)n);
		while (i != *pfree_i) {
			LAPJV_PRINTF("augment %d\n", j);
			LAPJV_PRINT_INDEX_ARRAY(pred, n);
			i = pred[j];
			LAPJV_PRINTF("y[%d]=%d -> %d\n", j, y[j], i);
			y[j] = i;
			LAPJV_PRINT_INDEX_ARRAY(x, n);
			LAPJV_SWAP_INDICES(j, x[i]);
			k++;
			if (k >= n) {
				LAPJV_ASSERT(LAPJV_FALSE);
			}
		}
	}
	LAPJV_FREE(pred);
	...
	int_t *x, int_t *y)
{
	...
	LAPJV_NEW(free_rows, int_t, n);
	LAPJV_NEW(v, cost_t, n);
