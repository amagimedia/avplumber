...
//
// Matches the amagimedia/scene-cut-experiments ONNX scene-cut detector's documented
// preprocessing contract EXACTLY: RGB channel order, no ImageNet mean/std, plain
// /255 scaling. This differs from amagi_reframer.cu's kNV12_to_TCHW_rgb_fp32, which
// intentionally outputs [0,255] values for a different model family — hence a
// dedicated kernel here rather than reusing that one.
//
...
{
    // BT.709 limited-range approximation, matching the recorder's other CUDA
    // NV12->RGB preprocess kernels (see amagi_reframer.cu).
    float y = (float)y8;
