// NV12 CUDA frame -> TCHW RGB tensor.
// Float/half outputs use [0,255] values to match the existing temporal path.
#include <stdint.h>
...
{
    // Match the current CUDA decode path's BT.709 limited-range approximation.
    float y = (float)y8;
...
extern "C" __global__ void kNV12_to_TCHW_rgb_u8(
    const uint8_t* __restrict__ y_plane, size_t pitch_y,
    const uint8_t* __restrict__ uv_plane, size_t pitch_uv,
    uint8_t* __restrict__ out_tchw,
    int width, int height,
    int bgr_order)
{
    const int x = (int)(blockIdx.x * blockDim.x + threadIdx.x);
    ...
    const int idx = y * width + x;
    if (bgr_order) {
        out_tchw[idx + 0 * plane_size] = (uint8_t)b;
        out_tchw[idx + 1 * plane_size] = (uint8_t)g;
        out_tchw[idx + 2 * plane_size] = (uint8_t)r;
    } else {
        out_tchw[idx + 0 * plane_size] = (uint8_t)r;
        out_tchw[idx + 1 * plane_size] = (uint8_t)g;
        out_tchw[idx + 2 * plane_size] = (uint8_t)b;
    }
}
