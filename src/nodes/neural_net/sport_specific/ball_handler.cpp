...
class BallHandler : public NodeSISO<av::VideoFrame, av::VideoFrame>, public ReportsFinishByFlag {
    ...
    std::string association_metadata_key_ = "ball_player_association";
    ...
    float min_association_ball_iou_ = 1e-12f;
    float min_association_score_ = -1.0f;
    float player_min_conf_ = 0.25f;
    int hysteresis_frames_ = 12;          // hold handler tag for N frames (~400ms) — survives dribble bounce
    bool prefer_current_handler_association_ = true;
    ...
    uint64_t stat_association_used_ = 0;
    ...
    struct AssociationCandidate {
        int player_id = -1;
        float score = 0.0f;
        float distance_px = std::numeric_limits<float>::max();
        float ball_iou = 0.0f;
        int rank = -1;
    };
    ...
    int findPlayerIndexByTrackId(const std::vector<Detection>& players, int track_id) const {
        if (track_id < 0) return -1;
        for (int i = 0; i < (int)players.size(); ++i) {
            if (players[i].has_track_id && players[i].track_id == track_id) return i;
        }
        return -1;
    }

    bool associationPlausible(const AssociationCandidate& c) const {
        if (c.distance_px <= max_distance_px_) return true;
        if (min_association_ball_iou_ > 0.0f && c.ball_iou >= min_association_ball_iou_) return true;
        if (min_association_score_ >= 0.0f && c.score >= min_association_score_) return true;
        return false;
    }

    std::vector<AssociationCandidate> parseAssociations(const AVFrame* raw) const {
        std::vector<AssociationCandidate> out;
        if (!raw || !raw->metadata || association_metadata_key_.empty()) return out;
        AVDictionaryEntry* entry = av_dict_get(raw->metadata, association_metadata_key_.c_str(), nullptr, 0);
        if (!entry || !entry->value) return out;
        try {
            Parameters md = Parameters::parse(entry->value);
            if (!md.contains("associations") || !md["associations"].is_array()) return out;
            for (const auto& item : md["associations"]) {
                if (!item.is_object()) continue;
                AssociationCandidate c;
                c.player_id = item.value("player_id", -1);
                c.score = item.value("score", 0.0f);
                c.distance_px = item.value("distance_px", std::numeric_limits<float>::max());
                c.ball_iou = item.value("ball_iou", 0.0f);
                c.rank = item.value("rank", -1);
                if (c.player_id >= 0) out.push_back(c);
            }
        } catch (...) {
            return {};
        }
        std::sort(out.begin(), out.end(), [](const AssociationCandidate& a, const AssociationCandidate& b) {
    ...
    bool pickAssociatedPlayer(const AVFrame* raw,
                              const std::vector<Detection>& players,
                              int& best_idx,
                              float& best_dist,
                              float& best_assoc_score,
                              float& best_ball_iou) const {
        std::vector<AssociationCandidate> candidates = parseAssociations(raw);
        if (candidates.empty()) return false;

        if (prefer_current_handler_association_ && current_handler_track_id_ >= 0) {
            for (const auto& c : candidates) {
                if (c.player_id != current_handler_track_id_ || !associationPlausible(c)) continue;
                const int idx = findPlayerIndexByTrackId(players, c.player_id);
                if (idx < 0) continue;
                best_idx = idx;
                best_dist = c.distance_px;
                best_assoc_score = c.score;
                best_ball_iou = c.ball_iou;
                return true;
            }
        }

        for (const auto& c : candidates) {
            if (!associationPlausible(c)) continue;
            const int idx = findPlayerIndexByTrackId(players, c.player_id);
            if (idx < 0) continue;
            best_idx = idx;
            best_dist = c.distance_px;
...
public:
    ...
    ~BallHandler() {
        ...
        logstream << "  association used:      " << stat_association_used_;
    }
    ...
    void process() override {
        ...
        float best_assoc_score = -1.0f;
        float best_ball_iou = 0.0f;
        bool used_association = false;
        if (have_ball) {
            used_association = pickAssociatedPlayer(raw, players, best_idx, best_dist, best_assoc_score, best_ball_iou);
            if (!used_association) {
                for (int i = 0; i < (int)players.size(); i++) {
        ...
        bool found_handler = used_association || (best_idx >= 0 && best_dist <= max_distance_px_);
        ...
        if (found_handler) {
            ...
            if (used_association) stat_association_used_++;
        ...
        if (emit_handler) {
            ...
            if (used_association) {
                det["association_source"] = "ball_player_association";
                det["association_score"] = best_assoc_score;
                det["association_ball_iou"] = best_ball_iou;
            }
        ...
        if (debug_log_every_n_ > 0 && (frame_counter_ % (uint64_t)debug_log_every_n_) == 0) {
            logstream << "ball_handler: frame=" << frame_counter_
                      ...
                      << " assoc=" << (used_association ? 1 : 0)
                      << " assoc_score=" << best_assoc_score
                      << " assoc_ball_iou=" << best_ball_iou
    ...
    static std::shared_ptr<BallHandler> create(NodeCreationInfo& nci) {
        ...
        if (params.count("association_metadata_key")) r->association_metadata_key_ = params["association_metadata_key"].get<std::string>();
        ...
        if (params.count("min_association_ball_iou")) r->min_association_ball_iou_ = params["min_association_ball_iou"];
        if (params.count("min_association_score")) r->min_association_score_ = params["min_association_score"];
        ...
        if (params.count("prefer_current_handler_association")) r->prefer_current_handler_association_ = params["prefer_current_handler_association"];
