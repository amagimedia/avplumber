...
constexpr const char* kNode = "v210_to_cuda";

void checkCuda(CUresult result, const char* operation) {
    v210cuda::check(result, kNode, operation);
}
...
av::Rational positiveRatio(const std::string& value) {
    return v210cuda::positiveRatio(value, kNode);
}
...
class V210ToCuda : public NodeSISO<av::Packet, av::VideoFrame>, public ReportsFinishByFlag,
    ...
    void initialize() {
        ...
        frames->width = layout_.width;
        frames->height = layout_.height;
        ...
        checkCuda(cuStreamCreate(&gpu_.stream, kNonBlockingStream), "create stream");
        checkCuda(cuMemAlloc(&gpu_.packed, layout_.size), "allocate packed buffer");
        checkCuda(cuMemHostAlloc(&gpu_.staging, layout_.size, 0), "allocate upload staging");
...
public:
    V210ToCuda(std::unique_ptr<SourceType>&& source, std::unique_ptr<SinkType>&& sink,
        ...
        : NodeSISO(std::move(source), std::move(sink)),
          layout_(v210cuda::makeLayout(kNode, params.at("width").get<int>(),
                                       params.at("height").get<int>(),
                                       params.value("stride", int64_t(0)))),
        ...
        format_ = av_get_pix_fmt(params.value("sw_format", std::string("p210le")).c_str());
        if (!v210cuda::isSupportedFormat(format_))
    ...
    void process() override {
        ...
        if (packet.size() != layout_.size || !packet.data())
        ...
        av::VideoFrame output;
        {
            ...
            std::memcpy(gpu_.staging, packet.data(), layout_.size);
            auto* frame = output.raw();
            int semiplanar = v210cuda::isSemiplanar(format_);
            void* args[] = {&gpu_.packed, &layout_.stride, &layout_.width, &layout_.height,
                            &frame->data[0], &frame->linesize[0],
                            &frame->data[1], &frame->linesize[1],
                            &frame->data[2], &frame->linesize[2], &semiplanar};
            try {
                checkCuda(cuMemcpyHtoDAsync(gpu_.packed, gpu_.staging, layout_.size, gpu_.stream), "upload");
                checkCuda(cuLaunchKernel(gpu_.kernel, (layout_.width / 2 + 31) / 32, (layout_.height + 7) / 8, 1,
    ...
    int width() override { return layout_.width; }
    int height() override { return layout_.height; }
